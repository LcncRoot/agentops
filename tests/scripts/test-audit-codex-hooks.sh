...
EOF
  ...
  if ! jq -e '[.hooks | to_entries[] | .value[] | .hooks[]] | length == 23' "$codex_home/hooks.json" >/dev/null; then
    fail "repair did not restore the canonical handler set"
    return
  fi
  if ! jq -e '[.hooks | to_entries[] | .value[] | .hooks[] | select(.command | contains("/tmp/agentops-plugin/hooks/"))] | length == 23' \
    "$codex_home/hooks.json" >/dev/null; then
    fail "repair did not render commands with the active plugin root"
    return
  fi
  if ! jq -e '.total_handlers == 23 and .agentops_handlers == 23 and .foreign_handlers == 0' <<< "$output" >/dev/null; then
